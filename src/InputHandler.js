...
export class InputHandler {
  ...
  resetSelection(errorIndex = -1) {
    if (this.engine.combo > 1) {
      // Provide loud visual feedback if a big combo drops!
      if (this.engine.combo >= 3) {
        this.engine.popups.showPopup('COMBO BROKEN!', this.engine.ui.canvas.width / 2, this.engine.ui.canvas.height / 2, '#ff3366');
      }
      this.engine.dispatch('SET_COMBO', 1);
      
      // Instantly hide the combo meter from the HUD!
    ...
    this.engine.popups.triggerErrorFlash(errorIndex !== -1 ? errorIndex : null);
    ...
    const penaltyMs = Math.min(
      gs.MAX_ERROR_PENALTY_MS,
      gs.MIN_ERROR_PENALTY_MS + (this.engine.level - 1) * 500
    );
    this.engine.dispatch('SUBTRACT_TIME', penaltyMs);
    
    // Show visual penalty indicator right where the finger messed up
    const popupIndex = errorIndex !== -1 ? errorIndex : (this.engine.selectedBlocks.length > 0 ? this.engine.selectedBlocks[this.engine.selectedBlocks.length - 1] : -1);
    if (popupIndex !== -1) {
      const center = this.engine.grid.getBlockCenter(popupIndex);
      if (center) this.engine.popups.showPopup(`-${(penaltyMs / 1000).toFixed(1)}s`, center.x, center.y, '#ff3366');
    } else {
      this.engine.popups.showPopup(`-${(penaltyMs / 1000).toFixed(1)}s`, this.engine.pointerPos.x, this.engine.pointerPos.y, '#ff3366');
    }
  ...
  _validatePathContinuity(tipIndex, unvisitedIndices, depth = 0, remainingCount = unvisitedIndices.length) {
    ...
    for (let i = 0; i < unvisitedIndices.length; i++) {
      ...
      if (nextIndex !== -1 && isAdjacent(tipIndex, nextIndex, this.engine.config.level.GRID_COLS)) {
  ...
  updatePathState() {
    ...
    const isTipEncrypted = this.engine.grid.isBlockEncrypted(tipIndex);

    this.engine.isPointerComplete = !this.engine.isPointerInvalid && (this.engine.selectedBlocks.length === currentTarget.size) && !isTipEncrypted;
  ...
  completePath() {
    ...
    const rewards = this.engine.config.tuning.TIME_REWARDS;
    const timeBonus = (clearedCount * rewards.TIME_BONUS_PER_BLOCK_MS) + (this.engine.combo * rewards.TIME_BONUS_PER_COMBO_MS);
    this.engine.dispatch('ADD_TIME', timeBonus);

    if (this.engine.currentDragColorHex && this.engine.selectedBlocks.length > 0) {
      const lastIndex = this.engine.selectedBlocks[this.engine.selectedBlocks.length - 1];
      const center = this.engine.grid.getBlockCenter(lastIndex);
      if (center) this.engine.popups.showPopup(`+${(timeBonus / 1000).toFixed(1)}s`, center.x, center.y, this.engine.currentDragColorHex);
    }

    // Advance the internal combo multiplier for exponentially better time bonuses
    this.engine.dispatch('INCREMENT_COMBO');
    
    // Orb Shatter VFX: Request safe coordinates from the HUDManager to maintain encapsulation
    const orbCenter = this.engine.hud.getHUDOrbCenter();
    if (orbCenter && this.engine.currentDragColorHex) {
      this.engine.particles.spawn(orbCenter.x, orbCenter.y, this.engine.currentDragColorHex, 15, 12);
    }

    this.engine.selectedBlocks.forEach(idx => {
      const center = this.engine.grid.getBlockCenter(idx);
      if (this.engine.currentDragColorHex && center) {
        this.engine.particles.spawn(center.x, center.y, this.engine.currentDragColorHex, 5, 5);
      }
      this.engine.grid.setBlockEmpty(idx);
    });

    if (navigator.vibrate) navigator.vibrate(this.engine.config.tuning.VIBRATION.SUCCESS);
  ...
  handlePointerDown(index, x, y) {
    ...
    if (this.engine.selectedBlocks.length > 0) {
      ...
      if (index === lastIndex) {
        ...
        this.engine.dispatch('SET_DRAGGING', true);
        ...
        if (navigator.vibrate) navigator.vibrate(this.engine.config.tuning.VIBRATION.DESELECT);
        this.engine.audio?.playDeselect?.();
        return;
      } else if (blockData.colorClass === currentInfo.class && isAdjacent(lastIndex, index, this.engine.config.level.GRID_COLS) && !this.engine.selectedBlocks.includes(index)) {
        
        if (this.engine.hazards.getPlugin('phasing')?.isPhasedOut(index)) {
  ...
  handlePointerMove(index, x, y) {
    ...
    if (blockData.colorClass === this.engine.currentDragColorClass) {
      if (!this.engine.selectedBlocks.includes(index)) {
        const tipIndex = this.engine.selectedBlocks[this.engine.selectedBlocks.length - 1];
        if (this.engine.hazards.getPlugin('phasing')?.isPhasedOut(index)) {
          this.engine.isPointerInvalid = true; // Passing over a phased out block is deadly!
        } else if (this.engine.grid.isBlockEncrypted(tipIndex)) {
          this.engine.isPointerInvalid = true; // Error! Trying to swipe past a locked node before decrypting!
        } else if (isAdjacent(tipIndex, index, this.engine.config.level.GRID_COLS)) {
          this.addBlock(index);
        } else {
  ...
  handlePointerUp() {
    ...
    if (this.engine.selectedBlocks.length === requiredCount && requiredCount > 0 && !isTipEncrypted) { 
      this.completePath();
