...
export class GridManager {
   ...
   * @param {number} rows 
   */
  setupGridDimensions(cols, rows) {
    this.ui.gridContainer.style.gridTemplateColumns = `repeat(${cols}, 1fr)`;
    this.ui.gridContainer.style.gridTemplateRows = `repeat(${rows}, 1fr)`;
  }

  /**
   * Toggles the memory leak blackout state on the grid container.
   * @param {boolean} isActive 
   */
  setMemoryLeakState(isActive) {
    if (isActive) this.ui.gridContainer.classList.add('memory-leak-active');
    else this.ui.gridContainer.classList.remove('memory-leak-active');
  }
  ...
  setBlockEmpty(index) {
    ...
    if (block) {
      ...
      block.dataset.phaseOffset = '';
      block.style.opacity = '';
      block.style.filter = '';
      block.dataset.isEncrypted = 'false';
      block.classList.remove('encrypted-node');
    }
  }
   ...
   */
  setWarningShake(indices, isWarning) {
    indices.forEach(idx => {
      const b = this.ui.gridContainer.children[idx];
      if (b) {
        if (isWarning) b.classList.add('warning-shake');
        else b.classList.remove('warning-shake');
      }
    });
  }
  ...
  updateChameleonBlocks(isCycling) {
    ...
    for (let i = 0; i < allBlocks.length; i++) {
      ...
      if (blockEl.dataset.isChameleon === 'true' && blockEl.dataset.colorClass !== 'empty') {
        if (isCycling) blockEl.style.backgroundColor = this.config.level.ALL_COLORS[Math.floor(Math.random() * this.config.level.ALL_COLORS.length)].hex;
   ...
   * @param {number} index 
   * @returns {{index: number, colorClass: string, colorHex: string | null, isChameleon: boolean, phaseOffset: number | null, isEncrypted: boolean} | null}
   */
  getBlockData(index) {
    ...
    return {
      ...
      phaseOffset: blockEl.dataset.phaseOffset ? parseFloat(blockEl.dataset.phaseOffset) : null,
      isEncrypted: blockEl.dataset.isEncrypted === 'true'
    };
  }
  ...
  /**
   * Assigns a random phase offset to a subset of blocks, making them vulnerable to the Phasing Hazard.
   * @param {() => number} rng 
   * @param {number} level 
   * @param {any} progression
   */
  assignPhasingOffsets(rng, level, progression) {
    const allBlocks = this.ui.gridContainer.children;
    const candidates = [];
    
    // 1. Reset all blocks and collect eligible candidates
    for (let i = 0; i < allBlocks.length; i++) {
      const b = /** @type {HTMLElement} */ (allBlocks[i]);
      b.dataset.phaseOffset = '';
      b.style.opacity = '';
      b.style.filter = '';
      if (b.dataset.colorClass !== 'empty') {
        candidates.push(b);
      }
    }

    // 2. Slow ramp-up based on the configured unlock level
    const maxPhased = Math.min(3, 1 + Math.floor((level - progression.hazards.phasing.unlockLevel) / 10));
    for (let i = 0; i < maxPhased && candidates.length > 0; i++) {
      const randIdx = Math.floor(rng() * candidates.length);
      const b = candidates.splice(randIdx, 1)[0];
      b.dataset.phaseOffset = (rng() * Math.PI * 2).toString();
    }
  }

  /**
   * @returns {Array<{index: number, offset: number}>}
   */
  getPhasingBlocks() {
    const phasing = [];
    const allBlocks = this.ui.gridContainer.children;
    for (let i = 0; i < allBlocks.length; i++) {
      const b = /** @type {HTMLElement} */ (allBlocks[i]);
      if (b.dataset.phaseOffset) phasing.push({ index: i, offset: parseFloat(b.dataset.phaseOffset) });
    }
    return phasing;
  }

  /**
   * @param {Array<{index: number, opacity: number}>} opacities 
   */
  applyOpacities(opacities) {
    for (const op of opacities) {
      const b = /** @type {HTMLElement} */ (this.ui.gridContainer.children[op.index]);
      if (b) {
        b.style.opacity = op.opacity.toString();
        b.style.filter = op.opacity < 0.4 ? 'saturate(200%) blur(2px)' : '';
      }
    }
  }

  /**
   * Locks random target blocks behind decryption.
   * @param {() => number} rng 
   * @param {number} level 
   * @param {any} progression 
   */
  assignEncryptedNodes(rng, level, progression) {
    const allBlocks = this.ui.gridContainer.children;
    const candidates = [];
    for (let i = 0; i < allBlocks.length; i++) {
      const b = /** @type {HTMLElement} */ (allBlocks[i]);
      b.dataset.isEncrypted = 'false';
      b.classList.remove('encrypted-node');
      if (b.dataset.colorClass !== 'empty' && b.dataset.colorClass !== 'color-stone') {
        candidates.push(b);
      }
    }
    const maxEncrypted = Math.min(3, 1 + Math.floor((level - progression.hazards.encryptedNodes.unlockLevel) / 10));
    for (let i = 0; i < maxEncrypted && candidates.length > 0; i++) {
      const randIdx = Math.floor(rng() * candidates.length);
      const b = candidates.splice(randIdx, 1)[0];
      b.dataset.isEncrypted = 'true';
      b.classList.add('encrypted-node');
    }
  }
  ...
  getEncryptedBlocks() {
    ...
    for (let i = 0; i < allBlocks.length; i++) {
      const b = /** @type {HTMLElement} */ (allBlocks[i]);
      if (b.dataset.isEncrypted === 'true') encrypted.push(i);
    }
    return encrypted;
  }
   ...
   * @returns {boolean}
   */
  isBlockEncrypted(index) {
    const b = /** @type {HTMLElement} */ (this.ui.gridContainer.children[index]);
    return b ? b.dataset.isEncrypted === 'true' : false;
  }

  /**
   * @param {number} index 
   */
  setBlockDecrypted(index) {
    const b = /** @type {HTMLElement} */ (this.ui.gridContainer.children[index]);
    if (b) {
      b.dataset.isEncrypted = 'false';
      b.classList.remove('encrypted-node');
  ...
  initGrid(targetQueue, level = 1, rng = Math.random, config) {
    ...
    for (let i = 0; i < targetQueue.length; i++) {
      ...
      for (let pos of t.cells) {
        ...
        if (t.colorInfo.class === 'color-stone') {
            block.style.backgroundImage = 'radial-gradient(#777, #333)';
            block.style.border = '2px solid #222';
        } else if (level === config.progression.milestones.bossLevel || (level >= config.progression.hazards.chameleon.unlockLevel && rng() < 0.08)) {
    ...
    for (let i = 0; i < GRID_COLS * GRID_ROWS; i++) {
      ...
      blocks[i].dataset.phaseOffset = '';
      blocks[i].style.opacity = '';
      blocks[i].style.filter = '';
      blocks[i].dataset.isEncrypted = 'false';
      blocks[i].classList.remove('encrypted-node');
   ...
   */
  processGravity() {
    const GRID_COLS = this.config.level.GRID_COLS;
    const GRID_ROWS = this.config.level.GRID_ROWS;
    /** @type {Array<Array<{class: string, hex: string | null, isChameleon: boolean, phaseOffset: string, isEncrypted: boolean}>>} */
    ...
    for (let x = 0; x < GRID_COLS; x++) {
      for (let y = 0; y < GRID_ROWS; y++) {
        ...
        if (block.dataset.colorClass !== 'empty') {
          columns[x].push({ class: block.dataset.colorClass || 'empty', hex: block.dataset.colorHex || null, isChameleon: block.dataset.isChameleon === 'true', phaseOffset: block.dataset.phaseOffset || '', isEncrypted: block.dataset.isEncrypted === 'true' });
        }
      }
      while (columns[x].length < GRID_ROWS) columns[x].unshift({ class: 'empty', hex: '#000000', isChameleon: false, phaseOffset: '', isEncrypted: false });
      ...
      for (let y = 0; y < GRID_ROWS; y++) {
        ...
        block.dataset.phaseOffset = newData.phaseOffset;
        block.dataset.isEncrypted = newData.isEncrypted ? 'true' : 'false';
        
        if (newData.phaseOffset === '') {
          block.style.opacity = '';
          block.style.filter = '';
        }
        
        if (newData.isEncrypted) block.classList.add('encrypted-node');
        else block.classList.remove('encrypted-node');
